...
class Mailer < ActionMailer::Base
  ...
  def issue_add(issue, to_users, cc_users)
    ...
    mail :to => to_users,
      :cc => cc_users,
      :subject => "DEVELOPMENT REPLICA: [#{issue.project.name} - #{issue.tracker.name} ##{issue.id}] (#{issue.status.name}) #{issue.subject}"
  end
  ...
  def issue_edit(journal, to_users, cc_users)
    ...
    references issue
    @author = journal.user
    s = "DEVELOPMENT REPLICA: [#{issue.project.name} - #{issue.tracker.name} ##{issue.id}] "
  ...
  def document_added(document)
    ...
    mail :to => document.notified_users,
      :subject => "DEVELOPMENT REPLICA: [#{document.project.name}] #{l(:label_document_new)}: #{document.title}"
  end
  ...
  def attachments_added(attachments)
    ...
    mail :to => recipients,
      :subject => "DEVELOPMENT REPLICA: [#{container.project.name}] #{l(:label_attachment_new)}"
  end
  ...
  def news_added(news)
    ...
    mail :to => news.notified_users,
      ...
      :subject => "DEVELOPMENT REPLICA: [#{news.project.name}] #{l(:label_news)}: #{news.title}"
  end
  ...
  def news_comment_added(comment)
    ...
    mail :to => news.notified_users,
     ...
     :subject => "DEVELOPMENT REPLICA: Re: [#{news.project.name}] #{l(:label_news)}: #{news.title}"
  end
  ...
  def message_posted(message)
    ...
    mail :to => recipients,
      :cc => cc,
      :subject => "DEVELOPMENT REPLICA: [#{message.board.project.name} - #{message.board.name} - msg#{message.root.id}] #{message.subject}"
  end
  ...
  def wiki_content_added(wiki_content)
    ...
    mail :to => recipients,
      :cc => cc,
      :subject => "DEVELOPMENT REPLICA: [#{wiki_content.project.name}] #{l(:mail_subject_wiki_content_added, :id => wiki_content.page.pretty_title)}"
  end
  ...
  def wiki_content_updated(wiki_content)
    ...
    mail :to => recipients,
      :cc => cc,
      :subject => "DEVELOPMENT REPLICA: [#{wiki_content.project.name}] #{l(:mail_subject_wiki_content_updated, :id => wiki_content.page.pretty_title)}"
  end
  ...
  def security_notification(recipients, options={})
    ...
    mail :to => recipients,
      :subject => "DEVELOPMENT REPLICA: [#{Setting.app_title}] #{l(:mail_subject_security_notification)}"
  end
  ...
  def test_email(user)
    ...
    mail :to => user.mail,
      :subject => 'DEVELOPMENT REPLICA: Redmine test'
  end
