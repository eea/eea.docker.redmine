...
SOLID_QUEUE_ROLE=${SOLID_QUEUE_ROLE:-}
SOLID_QUEUE_ONLY_WORK=${SOLID_QUEUE_ONLY_WORK:-0}
SOLID_QUEUE_SKIP_RECURRING=${SOLID_QUEUE_SKIP_RECURRING:-0}
RAILS_MAX_THREADS=${RAILS_MAX_THREADS:-5}
SOLID_QUEUE_WORKER_THREADS=${SOLID_QUEUE_WORKER_THREADS:-${RAILS_MAX_THREADS}}
...
# SolidQueue role-based configuration: separates scheduler/dispatcher from workers
if [ -n "${SOLID_QUEUE_ROLE}" ]; then
  case "${SOLID_QUEUE_ROLE}" in
    scheduler)
      export SOLID_QUEUE_CONFIG="${REDMINE_PATH}/config/solid_queue_scheduler.yml"
      export SOLID_QUEUE_ONLY_WORK=0
      export SOLID_QUEUE_SKIP_RECURRING=0
      ;;
    worker)
      export SOLID_QUEUE_CONFIG="${REDMINE_PATH}/config/solid_queue_workers.yml"
      export SOLID_QUEUE_ONLY_WORK=1
      export SOLID_QUEUE_SKIP_RECURRING=1
      ;;
    *)
      echo "WARNING: Unknown SOLID_QUEUE_ROLE=${SOLID_QUEUE_ROLE}. Using default (all processes)."
      ;;
  esac
fi
